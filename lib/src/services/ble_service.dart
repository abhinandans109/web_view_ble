...
class BleService {
  ...
  Future<bool> connect(deviceId,controller,
  ...
  subscribeCharacteristics({
    ...
    QualifiedCharacteristic characteristic = QualifiedCharacteristic(
        ...
        serviceId: Uuid.parse(serviceUUID),
        deviceId: deviceID);
    print("#####LOGINFO###### "+"at point 1");
    StreamSubscription characteristicSubscription = flutterReactiveBle
        .subscribeToCharacteristic(characteristic)
        .listen((List<int> event) {
      print("#####LOGINFO###### "+"inside listen");
      num otaWrittenSize=0;
      for(int i = 0; i < event.length; i++) {
        otaWrittenSize += pow(256, i) * event[i];
      }
      print('Decimal value characterstic : $characteristicUUID');
      print('Decimal value : $otaWrittenSize');
